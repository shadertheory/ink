...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("usage: tmp_parse_record <file>\n", .{});
        return;
    }
    ...
    const slice = tokens.items[87..127];
    var parse = try ink.peg_parser.parse_from(alloc, slice, ink.peg.nonterminal_kind.record_block);
    defer parse.deinit();
    std.debug.print("record_block ok: {any}\n", .{parse.ok});
    if (!parse.ok) {
        if (parse.@"error") |info| {
