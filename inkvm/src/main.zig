...
const Cli = struct {
    ...
    fn print_usage(p: *std.Io.Writer, exe_name: []const u8) !void {
        try p.print("Usage: {s} [--sim] [--scenario <name>] [--all] [--seed <n>] [--samples <n>] [--replay <path>] <program.inkb>\n", .{exe_name});
    }
};
    ...
    if (options.sim) {
        ...
        if (worker_count <= 1) {
            for (runs.runs) |*run| {
                ...
                const run_result = ink.vm.runtime.run_program_with_config(allocator, &program, lib_dir, run_cfg);
                if (run_result) |_| {
                    if (capture_report) {
                        reports.append(report) catch |err| {
