...
pub fn parse(allocator: std.mem.Allocator, manifest_path: []const u8) !Manifest {
    ...
    for (deps.items) |d| {
        ...
        if (d.path.len == 0 and d.registry.len == 0 and default_registry == null) {
    ...
    if (prelude_defined) {
        const std_items = try prelude_std_items.toOwnedSlice();
        const std_scopes = try prelude_std_scopes.toOwnedSlice();
        prelude = .{ .std_items = std_items, .std_scopes = std_scopes };
    }

    return .{
        .name = name.?,
        .version = version.?,
        .registry = default_registry orelse "",
        .root_dir = root_dir,
...
fn count_indent(text: []const u8) usize {
    var count: usize = 0;
    var idx: usize = 0;
...
fn parse_assignment(text: []const u8) ?assignment {
    const idx = std.mem.indexOf(u8, text, "=") orelse return null;
    const key = std.mem.trim(u8, text[0..idx], " \t");
    const raw_val = std.mem.trim(u8, text[idx + 1 ..], " \t");
...
fn append_prelude_list(list: *array_list([]const u8), value: []const u8) !void {
