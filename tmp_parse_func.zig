...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("usage: tmp_parse_func <file>\n", .{});
        return;
    }
    ...
    const slice = tokens.items[147..206];
    var parse = try ink.peg_parser.parse_from(alloc, slice, ink.peg.nonterminal_kind.function_decl);
    defer parse.deinit();
    std.debug.print("function_decl ok: {any}\n", .{parse.ok});
    if (!parse.ok) {
        if (parse.@"error") |info| {
