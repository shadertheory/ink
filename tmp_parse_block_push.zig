...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("usage: tmp_parse_block_push <file>\n", .{});
        return;
    }
    ...
    const slice = tokens.items[156..205];
    var parse = try ink.peg_parser.parse_from(alloc, slice, ink.peg.nonterminal_kind.block);
    defer parse.deinit();
    std.debug.print("push block ok: {any}\n", .{parse.ok});
    if (!parse.ok) {
        if (parse.@"error") |info| {
