...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("usage: tmp_parse_implbody <file>\n", .{});
        return;
    }
    ...
    const slice = tokens.items[66..144];
    var parse = try ink.peg_parser.parse_from(alloc, slice, ink.peg.nonterminal_kind.impl_body);
    defer parse.deinit();
    std.debug.print("impl_body ok: {any}\n", .{parse.ok});
    if (!parse.ok) {
        if (parse.@"error") |info| {
