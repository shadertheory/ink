...
fn load_module_sources(graph: *Graph, path: []const u8) ![]ink.compiler.source_id {
    ...
    var dir = std.fs.cwd().openDir(path, .{ .iterate = true }) catch return error.MissingSources;
    defer dir.close();
...
fn load_source_file(
    graph: *Graph,
    path: []const u8,
    ids: *array_list(ink.compiler.source_id),
) !void {
    const text = try std.fs.cwd().readFileAlloc(graph.allocator, path, 1_000_000);
    const id = graph.next_source_id;
    graph.next_source_id += 1;
    const stored_path = try dupe_path(graph, path);
    try graph.sources.append(.{ .id = id, .path = stored_path, .text = text });
    try ids.append(id);
}

fn resolve_path(graph: *Graph, base_dir: []const u8, input: []const u8) ![]const u8 {
    if (std.fs.path.isAbsolute(input)) return dupe_path(graph, input);
    return alloc_path(graph, &.{ base_dir, input });
}

fn alloc_path(graph: *Graph, parts: []const []const u8) ![]const u8 {
    const path = try std.fs.path.join(graph.allocator, parts);
    try graph.owned_paths.append(path);
    return path;
}

fn dupe_path(graph: *Graph, value: []const u8) ![]const u8 {
    const duped = try graph.allocator.dupe(u8, value);
    try graph.owned_paths.append(duped);
    return duped;
}
...
fn contains_name(list: []const []const u8, target: []const u8) bool {
    for (list) |item| {
