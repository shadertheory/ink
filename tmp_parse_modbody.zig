...
pub fn main() !void {
    ...
    if (args.len < 2) {
        std.debug.print("usage: tmp_parse_modbody <file>\n", .{});
        return;
    }
    ...
    const slice = tokens.items[3..275];
    var parse = try ink.peg_parser.parse_from(alloc, slice, ink.peg.nonterminal_kind.mod_body);
    defer parse.deinit();
    std.debug.print("mod_body ok: {any}\n", .{parse.ok});
    if (!parse.ok) {
        if (parse.@"error") |info| {
